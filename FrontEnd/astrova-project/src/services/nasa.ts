...
export const getEpicDates = async (
  type: EpicType = 'natural',
): Promise<{ date: string }[]> => {
...
export const getEpicByDate = async (
  ...
  const { data } = await api.get(`/api/nasa/epic/${date}`, {
    params: { type },
  });
  return data.data;
};
...
export const getEarthImagery = async (
  ...
  const { data } = await api.get('/api/nasa/earth/imagery', {
...
export const getEarthAssets = async (
  lon: number,
  lat: number,
  date: string,
  dim?: number,
): Promise<EarthAssetsResponse> => {
  const { data } = await api.get('/api/nasa/earth/assets', {
    params: { lon, lat, date, dim },
  });
  return data.data;
};
...
): Promise<{ collection: { items: MediaAsset[] } }> => {
