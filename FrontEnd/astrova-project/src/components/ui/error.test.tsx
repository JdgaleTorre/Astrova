...
describe('ErrorDisplay', () => {
  it('renders error message for 404', () => {
    ...
    expect(
      screen.getByText('No astronomy picture found for this date'),
    ).toBeInTheDocument();
  });
  ...
  it('renders error message for 500', () => {
    ...
    expect(
      screen.getByText(
        "NASA's servers are experiencing issues. Please try again later",
      ),
    ).toBeInTheDocument();
  });
  ...
  it('forwards className', () => {
    const { container } = render(
      <ErrorDisplay error={new Error('test')} className="custom-class" />,
    );
