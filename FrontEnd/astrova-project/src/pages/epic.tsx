...
function RouteComponent() {
  ...
  if (datesAvailable)
    return (
      <EpicView initialDate={new Date(datesAvailable[0].date)} type={type} />
    );
...
function EpicView({
  ...
  const { data, isLoading: isImagesLoading } = useQuery({
    ...
    queryFn: () =>
      getEpicByDate(selectedDate.toISOString().split('T')[0], type),
  });
  ...
  if (!data || data.length === 0) {
    return (
      <div className="relative flex min-h-screen flex-col pt-16">
        ...
        <div className="container mx-auto flex max-w-7xl items-center justify-center px-4 py-12 md:pt-48">
